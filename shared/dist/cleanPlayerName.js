 ...
 */
const hexInvalidString = (str, limit = 35) => {
    const hexArr = [...str].map(c => (c.codePointAt(0) ?? 0x00).toString(16));
    const outArr = [];
    let charCountWithSpace = 0;
    for (let i = 0; i < hexArr.length; i++) {
        if (outArr.includes(hexArr[i]))
            continue;
        if ((charCountWithSpace + hexArr[i].length) > limit)
            break;
...
const isEmoji = (char) => {
    ...
    return ((codePoint >= 0x1F300 && codePoint <= 0x1F5FF) || // Miscellaneous Symbols and Pictographs
 ...
 */
const cleanTrimCodePoints = (str, lenLimit = 36) => {
    let out = '';
    let lastChar = '';
    let lastCharCount = 0;
...
exports.default = (originalName) => {
    ...
    const displaySteps = [
        ...
        (x) => x.replace(/(^\p{Nonspacing_Mark}+)|(\p{Nonspacing_Mark}{3,})/ug, (match, leading, repeating) => {
            if (leading)
                return ''; // Remove leading non-spacing marks
            if (repeating)
                return repeating.substring(0, 2); // Truncate sequences of three or more non-spacing marks to two
        }),
        // remove leading invisible characters

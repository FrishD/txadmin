...
import { sendLoginAttempt } from '@modules/DiscordBot/discordHelpers';
...
export default async function AuthVerifyPassword(ctx: InitializedCtx) {
    ...
    try {
        ...
        if (!vaultAdmin) {
            ...
            sendLoginAttempt(postBody.username, ctx.ip, ctx.request.headers['user-agent'], false);
            return ctx.send<ApiVerifyPasswordResp>({
        ...
        //Setting up session
        sendLoginAttempt(postBody.username, ctx.ip, ctx.request.headers['user-agent'], true);
        const sessData = {
            type: 'password',

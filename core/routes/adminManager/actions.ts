...
import { sendPermissionChange } from '@modules/DiscordBot/discordHelpers';
...
async function handleResetPassword(ctx: AuthedCtx) {
    ...
    if (ctx.admin.name.toLowerCase() === name.toLowerCase()) {
        return ctx.send({ type: 'danger', message: 'You cannot reset your own password.' });
    }

    //Add admin and give output
    try {
        const newPassword = await txCore.adminStore.resetPassword(name);
        ctx.admin.logAction(`Resetting password for user '${name}'.`);
        return ctx.send({ type: 'showPassword', password: newPassword });
    } catch (error) {
        return ctx.send({ type: 'danger', message: (error as Error).message });
    }
}
...
async function handleEdit(ctx: AuthedCtx) {
    ...
    const permsBefore = [...admin.permissions];
    ...
    try {
        ...
        sendPermissionChange(admin, ctx.admin, permsBefore, permissions);

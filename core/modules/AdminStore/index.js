...
export default class AdminStore {
    ...
    /**
     * Returns a list of admins with a specific permission.
     * @param {string} permission
     */
    getAdminsWithPermission(permission) {
        if (!Array.isArray(this.admins)) {
            return [];
        }
        return this.admins.filter((admin) => {
            return admin.master || admin.permissions.includes('all_permissions') || admin.permissions.includes(permission);
        }).map((admin) => {
            return {
                name: admin.name,
                providers: Object.keys(admin.providers),
            }
        });
    }
    ...
    async resetPassword(name) {
        ...
        if (this.admins[adminIndex].master) {
            throw new Error('You cannot reset the password of a master account.');
        }
    ...
    loadAdminsFile() {
        ...
        try {
            raw = fs.readFileSync(this.adminsFile, 'utf8');

...
import { embedColors, getDiscordBot } from './discordHelpers';
...
export default class DiscordBot {
    ...
    constructor() {
        ...
        // Assign to txCore after initialization
        setImmediate(() => {
            txCore.discordBot = this;
        });
            ...
            this.#client.on('guildMemberAdd', async (member) => {
                try {
                    ...
                    if (activeBlacklist.length && txConfig.discordBot.blacklistRole) {
                        ...
                        // Remove the specific role if they have it
                        if (txConfig.discordBot.blacklistRemoveRole) {
                            const hasRoleToRemove = member.roles.cache.has(txConfig.discordBot.blacklistRemoveRole);
                            if (hasRoleToRemove) {
                                await member.roles.remove(txConfig.discordBot.blacklistRemoveRole);
                                console.log(`Removed role ${txConfig.discordBot.blacklistRemoveRole} from blacklisted user ${member.user.tag}`);
                            }
                        }
                    }
                } catch (error) {
     ...
     */
    async syncWagerBlacklistRoles() {
        if (!txConfig.discordBot.wagerBlacklistRole) return;
        ...
        try {
            ...
            if (!this.guild) {
                console.error('Guild not found, cannot sync wager blacklist roles.');
                return;
            }
            ...
            for (const action of activeBlacklist) {
                ...
                if (discordId) {
                    ...
                    if (member && !member.roles.cache.has(txConfig.discordBot.wagerBlacklistRole)) {
                        await member.roles.add(txConfig.discordBot.wagerBlacklistRole);
                        console.log(`Added wager blacklist role to ${member.user.tag}.`);
                    }
                }
            }
            console.log('Wager blacklist role sync complete.');
        } catch (error) {
    ...
    async syncBlacklistRoles() {
        ...
        try {
            const allActions = txCore.database.actions.getRaw();
            const activeBlacklist = allActions.filter(a =>
                a.type === 'ban' &&
                'blacklist' in a &&
                a.blacklist &&
            ...
            for (const action of activeBlacklist) {
                ...
                if (discordId) {
                    ...
                    const member = this.guild.members.cache.get(uid);
                    if (member) {
                        // Add blacklist role if not present
                        if (!member.roles.cache.has(txConfig.discordBot.blacklistRole)) {
                            await member.roles.add(txConfig.discordBot.blacklistRole);
                            console.log(`Added blacklist role to ${member.user.tag}.`);
                        }

                        // ALWAYS remove the complementary role if they have it (even if they already have blacklist role)
                        if (txConfig.discordBot.complementaryRole && member.roles.cache.has(txConfig.discordBot.complementaryRole)) {
                            await member.roles.remove(txConfig.discordBot.complementaryRole);
                            console.log(`Removed complementary role from blacklisted user ${member.user.tag}.`);
                        }
                    }
                }
            }
    ...
    /**
     * Helper function to manage blacklist roles for a user
     * Call this whenever a user is added to or removed from blacklist
     */
    async updateBlacklistRoles(discordId: string, isBlacklisted: boolean) {
        if (!this.guild || !txConfig.discordBot.blacklistRole) return;
        ...
        try {
            ...
            if (isBlacklisted) {
                ...
                console.log(`Updated blacklist roles for ${member.user.tag} (blacklisted)`);
            } else {
                // Remove blacklist role
                if (member.roles.cache.has(txConfig.discordBot.blacklistRole)) {
                    await member.roles.remove(txConfig.discordBot.blacklistRole);
                }

                console.log(`Removed blacklist role from ${member.user.tag} (unblacklisted)`);
            }
        } catch (error) {
            console.error(`Failed to update blacklist roles for ${discordId}: ${(error as Error).message}`);
        }
    }
};

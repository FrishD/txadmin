...
### מערכת בודקי מחשב (PC Checkers)
מערכת זו מוסיפה פאנל וכלים ייעודיים לצוות בודקי המחשבים, המאפשרים להם לנהל בדיקות בצורה יעילה ומסודרת.

**הגדרת הרשאות:**
1.  כדי להעניק למשתמש גישה לפאנל בודקי המחשב, יש להיכנס ללשונית "Admins" בפאנל הניהול.
2.  יש לערוך את המשתמש הרצוי ולהעניק לו את ההרשאה `players.pc_checker`.
3.  לאחר שמירת השינויים, יופיע בתפריט הניווט של המשתמש קישור חדש בשם "PC Checker", אשר יוביל אותו לפאנל הייעודי.

**יכולות הפאנל:**
*   **זימון שחקן לבדיקה:**
    *   בודק המחשב יכול לבחור שחקן מתוך השרת ולזמן אותו לבדיקה.
    *   השחקן המזומן יקבל הודעה גדולה במרכז המסך שלא ניתנת לסגירה למשך 30 שניות.
    *   בהודעה יופיע שם הבודק והנחיות ברורות לגבי תהליך הבדיקה.
*   **מערכת דיווחים:**
    *   הבודקים יכולים למלא דוח בדיקה מפורט ישירות מהפאנל.
    *   הדוח כולל שדות עבור: הבודק המדווח, המפקח שנכח בבדיקה, תוצאת הבדיקה ("עבר" / "לא עבר"), הסבר על מה שהשחקן נתפס, ותמונה כהוכחה מסכמת.
    *   חיפוש שחקן לצורך דיווח יציג את כל החשבונות החלופיים (אלטים) הידועים של השחקן.
*   **קישור דוחות לבאנים:**
    *   במידה ודוח מסומן כ"לא עבר" והשחקן המדובר מקבל באן לצמיתות בסמוך לזמן הדיווח, המערכת תקשר אוטומטית בין הדוח לבין הבאן.
*   **ניהול דוחות:**
    *   ניתן להגדיר הרשאת `pc.manager` למנהלי הבודקים.
    *   מנהלים עם הרשאה זו יוכלו למחוק דוחות. כל מחיקה תתועד ותישלח ללוג ייעודי בדיסקורד.
*   **פקודת דיסקורד `!pctop`:**
    *   פקודה זו מציגה טבלה עם סיכום של כמות הבדיקות שכל בודק ביצע בשבעת הימים האחרונים.

### שיפורים במערכת הבאנים
*   **הוספת הוכחות:**
    *   בעת מתן באן (דרך הפאנל או המשחק), ניתן כעת לצרף קובץ תמונה או וידאו (עד 15MB) כהוכחה.
*   **עריכת באנים:**
    *   ניתן לערוך באנים קיימים כדי להוסיף הוכחות חדשות.
    *   ניתן לשנות את סיבת הבאן, כאשר הסיבה המקורית נשמרת בהיסטוריית הבאן.

### מערכת Blacklist (רשימה שחורה)
*   בעת מתן באן לצמיתות (Permanent), מופיעה אפשרות חדשה: "Add blacklist".
*   סימון אפשרות זו יעניק לשחקן רול ייעודי בדיסקורד (יש להגדיר את ID הרול בקונפיג).
*   המערכת מוודאת שהרול נשאר על השחקן, גם אם הוא יוצא ונכנס מחדש לשרת הדיסקורד, ומבצעת בדיקה חוזרת בכל ריסטארט של השרת.

### לוגים ותיעוד
*   **לוג חיפוש שחקנים:** כל פעם שאיש צוות מבצע חיפוש אחרי שחקן, המידע הבא נשלח לערוץ ייעודי בדיסקורד: מי חיפש (ID דיסקורד), מה היה החיפוש, תאריך ושעה.
*   **לוג מחיקת דוחות PC:** מחיקת דוח על ידי מנהל בודקים תתועד ותישלח לדיסקורד.

### הגבלת כמות באנים
*   כל איש צוות מוגבל למתן של עד שלושה (3) באנים בשעה. ניסיון לחרוג מהמגבלה יציג הודעת שגיאה.

### תיקונים נוספים
*   פיצ'ר ה-Spectate תוקן ועובד כראוי.

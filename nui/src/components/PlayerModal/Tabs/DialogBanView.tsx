...
const DialogBanView: React.FC = () => {
  ...
  useEffect(() => {
    fetchWebPipe<string[]>("/adminManager/getApprovers")
      .then(setApprovers)
      .catch((e) => {
        enqueueSnackbar(`Failed to load approvers: ${e.message}`, {
          variant: "error",
        });
      });
  }, []);
  ...
  const isLongBan = useMemo(() => {
    ...
    const dur = duration === 'custom' ? `${customDurLength} ${customDuration}` : duration;
    const match = dur.match(/(\d+)\s(hour|day|week|month)s?/);
    if(match){
      const value = parseInt(match[1]);
      const unit = match[2];
      if(unit === 'hour') durationSeconds = value * 3600;
      else if(unit === 'day') durationSeconds = value * 86400;
      else if(unit === 'week') durationSeconds = value * 604800;
      else if(unit === 'month') durationSeconds = value * 2592000;
    }
    return durationSeconds > 604800; // 7 days in seconds
  }, [duration, customDurLength, customDuration]);
  ...
  const handleBan = (e: React.FormEvent) => {
    ...
    if (isLongBan && !selectedApprover) {
      enqueueSnackbar("For bans longer than 1 week, you must select an approver.", {
        variant: "error",
      });
      return;
    }

    const finalReason = isLongBan
      ? `${trimmedReason}\n(Approved by: ${selectedApprover})`
      : trimmedReason;
      ...
      <form onSubmit={handleBan}>
        ...
        {isLongBan && (
          <TextField
            ...
            helperText="Bans over 1 week require an approval from a Staff Manager or higher."
            fullWidth
          >
            {approvers.map((name) => (
